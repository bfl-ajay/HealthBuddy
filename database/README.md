# HealthGram Database Schema

## Overview
The application uses **SQLite** as the local database for structured data storage. All data is stored locally on the device with proper relational tables.

## Database: healthgram.db
...
- `password` - User password (stored as plain text for demo - use hashing in production)
...
Stores blood pressure readings for users.
